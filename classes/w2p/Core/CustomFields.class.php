...
class w2p_Core_CustomFields
{
    ...
    public function __construct($m, $notUsed, $obj_id = null, $mode = 'edit',
        $published = 0)
    {
        $this->m = $m;
        ...
        $q->addWhere('field_module = \'' . $this->m . '\' AND field_page = \'' .
            $this->a . '\'');
        if ($published) {
        ...
        if ($rows == null) {
            ...
            foreach ($rows as $row) {
                switch ($row['field_htmltype']) {
                    case 'checkbox':
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldCheckbox($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
                        break;
                    case 'href':
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldWeblink($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
                    ...
                    case 'textarea':
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldTextArea($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
                    ...
                    case 'select':
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldSelect($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
                    ...
                    case 'label':
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldLabel($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
                    ...
                    case 'separator':
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldSeparator($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
                    ...
                    default:
                        $this->fields[$row['field_name']] = new
                            w2p_Core_CustomFieldText($row['field_id'],
                            $row['field_name'], $row['field_order'],
                            stripslashes($row['field_description']),
                            stripslashes($row['field_extratags']),
                            $row['field_order'], $row['field_published']);
    ...
    public function add($field_name, $field_description, $field_htmltype,
        $field_datatype, $field_extratags, $field_order, $field_published,
        &$error_msg)
    {
        global $db;
    ...
    public function update($field_id, $field_name, $field_description,
        $field_htmltype, $field_datatype, $field_extratags, $field_order,
        $field_published, &$error_msg)
    {
        global $db;
    ...
    public function store($object_id)
    {
        if (!count($this->fields) == 0) {
            ...
            foreach ($this->fields as $k => $notUsed) {
                ...
                if ($result) {
                    $store_errors .= 'Error storing custom field ' . $k .
                        ':' . $result;
                }
            }
    ...
    public function search($moduleTable, $moduleTableId, $moduleTableName, $keyword)
    {
        ...
        $q->addJoin('custom_fields_struct', 'cfs', 'cfs.field_id =
            cfv.value_field_id');
        $q->addJoin($moduleTable, 'm', 'm.' . $moduleTableId . ' =
            cfv.value_object_id');
    ...
    public static function getCustomFieldByModule($notUsed, $module, $objectId)
    {
        ...
        if ($canRead) {
            ...
            $q->leftJoin('custom_fields_values', 'cfv', 'cfv.value_field_id =
                cfs.field_id');
            $q->leftJoin('custom_fields_lists', 'cfl', 'cfl.list_option_id =
                cfv.value_intvalue');
            $q->addWhere("cfs.field_module = '$module'");
