...
UPDATE `task_log` SET `task_log_percent_complete` = IFNULL(
    (SELECT `task_percent_complete` FROM `tasks` 
        WHERE tasks.task_id = task_log_task
        GROUP BY tasks.task_id), 0
);

ALTER TABLE  `task_log` ADD  `task_log_task_end_date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `task_log_percent_complete`;

UPDATE `task_log` SET `task_log_task_end_date` = IFNULL(
    (SELECT `task_end_date` FROM `tasks` 
        WHERE tasks.task_id = task_log_task
        GROUP BY tasks.task_id), NOW()
);
