...
function w2plink_user_main() {
	$url = trim(pnModGetVar('w2plink', 'url'));
	$window = pnModGetVar('w2plink', 'use_window');
	$wrap = pnModGetVar('w2plink', 'use_postwrap');
	...
	if ($window) {
		...
		if ($wrap) {
			...
			header('Location: modules.php?op=modload&name=w2plink&file=index&url=' . $url);
		}
	}
	exit;
}
