...
<table border="0" cellpadding="4" cellspacing="0" width="100%" class="std view">
	<tr>
		...
		<td width="50%" valign="top">
			...
			<br><strong><?php echo $AppUI->_('Associated inactive users'); ?></strong>
			<table cellspacing="0" cellpadding="2" border="0" width="100%">
			<?php
				$users = w2PgetUsersList();
				foreach ($users as $user) {
					if (($user['contact_company'] == $company_id) && (!$perms->isUserPermitted($user['user_id']))) {
						$htmlHelper->stageRowData($user);
						echo '<tr>';
						echo $htmlHelper->createCell('user_name', $user['contact_display_name']);
						echo $htmlHelper->createCell('user_email', $user['contact_email']);
						echo '</tr>';
					}
				}
