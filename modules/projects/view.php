...
$denied = $object->getDeniedRecords($AppUI->user_id);
if (in_array($object_id, $denied)) {
    $AppUI->redirect(ACCESS_DENIED);
}

// get critical tasks (criteria: task_end_date)
$criticalTasks = ($object_id > 0) ? $object->getCriticalTasks($object_id) : null;
...
if ($canEdit) {
    $titleBlock->addButton('new link', '?m=links&a=addedit&project_id=' . $object_id);
    $titleBlock->addButton('new event', '?m=events&a=addedit&project_id=' . $object_id);
    $titleBlock->addButton('new file', '?m=files&a=addedit&project_id=' . $object_id);
    $titleBlock->addCrumb('?m=projects&a=addedit&project_id=' . $object_id, 'edit this project');
    if ($canDelete) {
        $titleBlock->addCrumbDelete('delete project', $canDelete);
    }
}
if (canAdd('tasks')) {
    $titleBlock->addButton('new task', '?m=tasks&a=addedit&task_project=' . $object_id);
}
$titleBlock->show();

$view = new \Web2project\Output\HTML\View($AppUI, $object, 'Project');
...
$tabBox = new CTabBox('?m=projects&a=view&project_id=' . $object_id, '', $tab);
$query_string = '?m=projects&a=view&project_id=' . $object_id;
