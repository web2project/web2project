...
// Look through the files associated to the project, looking for files of the categories
// 'Project Picture' and 'Main Project Picture'. If no 'Main Project Picture' category file
// is found but one or more of 'Project Picture' category are found then use the first as
// the main picture. Also make sure the files are pictures (JPEG, PNG, GIF).
$pic_files = CFile::getFileList($AppUI, 0, $project_id, 0, -1);

$proj_pics = array();
$main_pic = null;
foreach($pic_files as $file) {
	if (in_array($file['file_type'], array('image/jpeg', 'image/png', 'image/gif'))) {
		if ($file['file_category'] == '9999') {
			if (!isset($main_pic)) {
				$main_pic = $file;
				continue;
			}
		}
		if (($file['file_category'] == '9998') || ($file['file_category'] == '9999')) {
			$proj_pics[] = $file;
		}
	}
}
if (!isset($main_pic) && count($proj_pics)) {
	$main_pic = $proj_pics[0];
}
...
<tr>
	<td style="border: outset #d1d1cd 1px;background-color:#<?php echo $project->project_color_identifier; ?>" colspan="<?php echo isset($main_pic) ? 3 : 2; ?>" id="view-header">
	<?php
...
<tr>
	<?php
		if (isset($main_pic)) {
			echo '<td valign="top" class="view-column">';
			echo '<img src="fileviewer.php?file_id=' . $main_pic['file_id'] . '" alt="Main Project Picture" height="200px">';
			echo '</td>';
		}
	?>
...
<!-- TODO: we can't use the createCell helper here because it centers things while we need it left-aligned -->
        ...
        <?php
	// Shows the project pictures
	if (count($proj_pics)) {
		echo '<tr><td colspan="3"><strong>' . $AppUI->_('Project Pictures') . '</strong></td></tr>';
		echo '<tr><td valign="top" class="view-column" colspan="' . (isset($main_pic) ? 3 : 2) . '">';
		foreach ($proj_pics as $pic) {
			echo '<img src="fileviewer.php?file_id=' . $pic['file_id'] . '" alt="Project Picture" height="150px">';
		}
		echo '</tr></td>';
	}
