...
// To enable the use of inactive users as assignees to tasks the code was
// changed to make sure that an active user has 'view' rights for the tasks,
// or is the current user or is an inactive user.
...
foreach ($rows as $row) {
	if ($perms->isUserPermitted($row['user_id'])) {
		if (($perms->isUserPermitted($row['user_id'], 'tasks')) || $row['user_id'] == $AppUI->user_id) {
