...
<tr>
	<td><?php echo w2PshowImage( 'applet-48.png', 16, 16, '' ); ?></td>
	<td nowrap><h1><?php echo $AppUI->_('Tasks Organizer Wizard'); ?></h1></td>
	<td nowrap><img src="<?php echo w2PfindImage('shim.gif');?>" width="16" height="16" alt="" border="0"></td>
...
$proj = new CProject();
$project_filter_where = $proj->getAllowedSQL($AppUI->user_id, 'projects.project_id');
if (count($project_filter_where) > 0) {
	$q->addWhere('(' . implode(' AND ', $project_filter_where) . ')');
}
