...
# NOTE 2: If your web2Project PHP environment is correctly set up
# you don't need to add the @ and domain, it will get it from
# w2Pconfig[site_domain] key.
...
## the PHP environment.
die ("Gateway.pl requires the full path to the web2project config.php file as its only argument") if ($#ARGV != 0);
%config = ();
&check_config($ARGV[0]);

# Shortcuts for the email code
$app_root = $config{'base_url'};
$w2p_root = $config{'root_dir'};
...
sub check_config() {
  $w2p_conf = $_[0];
  open (PHPCONFIG, "<$w2p_conf")
    or die ("Cannot find web2Project configuration file!");
  while (<PHPCONFIG>) {
    if (/^\s*\$w2pconfig\[/i) {
      s/\s*;.*$//;
