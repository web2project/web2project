...
$contact_id = intval(w2PgetParam($_GET, 'contact_id', ''));
...
if (isset($_GET['contact_id']) && !(w2PgetParam($_GET, 'contact_id', '') == '')) {
	...
	// add an address
	$vcard->addAddress('', $contacts[0]['contact_address2'], $contacts[0]['contact_address1'], $contacts[0]['contact_city'], $contacts[0]['contact_state'], $contacts[0]['contact_zip'], $contacts[0]['contact_country']);
	//$vcard->addParam('TYPE', 'WORK');
	...
	header('Content-Disposition: attachment; filename=' . $contacts[0]['contact_first_name'] . $contacts[0]['contact_last_name'] . '.vcf');
	print_r($text);
} else {
